...
Deployments are handled by DevSSP plugins. Each deploy plugin registers a Connection (credentials and target) and one or more Deployment Methods. DevSSP controls the full chain: credentials, execution, status, and audit trail.

Plugins that need Connections (DevSSP calls them directly):
...
- **OpenTofu/Terraform** — apply (DevSSP worker runs tofu/terraform with plugin-managed credentials)
...
|----------|-------------------|---------|
| `platform` | DevSSP worker calls target API directly | Docker direct, K8s direct, Helm, OpenTofu apply |
| `gitops` | DevSSP pushes manifests to a repo, external controller syncs | K8s gitops, ArgoCD gitops |
...
For teams that already have their own deployment pipelines and manage credentials at the CI level, DevSSP does not orchestrate the deployment, only observes.

This decision was made due to the complexity of securely orchestrating deployments via CI. Scoping CI credentials per service requires provisioning per-service IAM roles, which is itself an infrastructure operation that needs privileged credentials — creating a chicken-and-egg problem. Centralizing deployment logic in reusable workflows avoids this but introduces OIDC trust policy scoping issues: GitHub Environments are per-repo, while DevSSP Environments are per-project, making it difficult to map cloud credentials to the correct project-environment boundary. Plugin-based deployments sidestep both problems — DevSSP already holds credentials for deploy plugins, and adding IaC plugins (OpenTofu, Terraform) is no different from Docker or Kubernetes plugins.

The team's Deployment pipeline handles deployment using its own credentials (org secrets, OIDC federation, etc.). After deploying, the pipeline notifies DevSSP via webhook using the `ssp-notify-deploy` action from the CI Steps repo:

```yaml
# In the team's CI pipeline, after their deploy step:
- uses: org/ci-steps/ssp-notify-deploy@main
  with:
    ssp_url: ${{ vars.SSP_URL }}
    ssp_token: ${{ secrets.SSP_WEBHOOK_TOKEN }}
    project: ${{ env.SSP_PROJECT }}
    service: ${{ env.SSP_SERVICE }}
    environment: prod
    artifact_tag: v1.2.3
    status: success
    commit: ${{ github.sha }}
...
DevSSP records the deployment event but did not cause it. The team owns their pipeline end-to-end. DevSSP provides visibility: deployment history, environment status, and promotion tracking still work.

When selecting a Deployment method for a Path, the option "CI-managed" means DevSSP will not attempt to deploy. It will only track deployments reported via webhook. This is suitable for teams that have existing CI/CD pipelines they do not want to replace.
...
- Promotions to other Environments are manual via DevSSP UI/API and use reusable artifacts from the original build.
