...
class EnvVarSaveView(LoginRequiredMixin, ProjectContributorMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        locked_keys = [v["key"] for v in (self.project.env_vars or []) if v.get("lock")]
        if key in locked_keys:
            messages.error(
                request,
                f'Variable "{key}" is locked at project level and cannot be overridden.',
            )
            return redirect(
                "projects:environment_detail",
                project_name=self.project.name,
