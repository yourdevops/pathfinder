...
3. **Maintain CSP compatibility for strong frontend security.** Use Alpine.js CSP build directives (`x-data`, `x-on`, `x-show`, `x-bind`, etc.) for all client-side interactivity — they work in HTMX-swapped partials without CSP nonce issues. The CSP build's expression parser only supports **single expressions** (no semicolons, no arrow functions, no shorthand method syntax like `init() {}`). For multi-step logic, register components via `Alpine.data()` using `function()` syntax (not arrow functions) and reference them from templates. **Placement rules**: only truly shared/reusable components go in `base.html` (see `dropdown`, `copyBtn`). Page-specific components go in that page's template via `{% block extra_head %}`. Do not use inline `<script>` tags in HTMX partials or create separate JS files for simple UI logic.
